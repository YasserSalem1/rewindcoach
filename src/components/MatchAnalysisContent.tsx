  ...
  const duoPartners = useMemo(() => {
    ...
    matchesWindow.forEach((match) => {
      ...
      teamMates.forEach((mate) => {
        const record = duoMap.get(mate.summonerName) ?? { games: 0, wins: 0 };
        record.games += 1;
        if (participant.win) record.wins += 1;
        duoMap.set(mate.summonerName, record);
      });
    });

    return Array.from(duoMap.entries())
      .sort((a, b) => b[1].games - a[1].games)
      .slice(0, 3)
      .map(([name, record]) => ({
        name,
  ...
  return (
    <div className="relative mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-6">
      ...
      <section className="relative overflow-hidden rounded-3xl border border-white/10 bg-slate-950/90 p-6 shadow-[0_0_50px_rgba(79,70,229,0.25)]">
        ...
        {analysis.lpTimeline.length > 0 && (
          <div className="mt-6">
            <Card className="border-white/10 bg-slate-950/85">
              <CardHeader className="flex flex-col gap-1">
                <div className="flex items-center justify-between">
                  <CardTitle>LP Trajectory (Est.)</CardTitle>
                  <span
                    className={cn(
                      "text-sm font-semibold",
                      analysis.lpTrendChange >= 0 ? "text-emerald-300" : "text-rose-300",
                    )}
                  >
                ...
                <CardDescription>
                  Estimated change across the last 20 ranked games (±15 LP per win/loss)
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="h-48">
                  <ResponsiveContainer>
                    <LineChart data={analysis.lpTimeline}>
                      <defs>
                      ...
                      <Tooltip
                        formatter={(value: number, _name, payload) => [
                          `${value} LP`,
                          payload?.payload?.result === "W" ? "Win" : "Loss",
                        ]}
                        contentStyle={{
                ...
                <p className="mt-3 text-xs text-slate-500">
                  LP values are approximated for coaching insights and may differ slightly from in-game totals.
                </p>
              </CardContent>
            </Card>
          </div>
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        <Card className="border-white/10 bg-slate-950/80 lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Trends pulled from your last 20 games: KDA, CS cadence, kill participation, and streaks</CardDescription>
          </CardHeader>
          <CardContent>
            {analysis.timelineData.length ? (
              <div className="space-y-6">
                <ScrollArea className="w-full rounded-2xl border border-white/5">
                  <div className="h-64 min-w-[960px] p-2">
                    <ResponsiveContainer>
      ...
      <section className="grid gap-6 md:grid-cols-2">
        ...
        <Card className="border-white/10 bg-slate-950/80">
          <CardHeader>
            <CardTitle>Lane Frequency</CardTitle>
            <CardDescription>Where you queued in this sample</CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col gap-3">
            {analysis.laneFocus.length ? (
              analysis.laneFocus.map((lane) => (
                <div key={lane.lane} className="flex items-center gap-4">
                  <div className="w-28 text-sm text-slate-300">{lane.lane}</div>
                  <div className="flex-1 overflow-hidden rounded-full bg-slate-800/60">
                    <div
                      className="h-3 bg-gradient-to-r from-violet-500 to-fuchsia-500"
                      style={{ width: `${Math.round(lane.share * 100)}%` }}
                    />
                  </div>
                  <p className="w-12 text-right text-sm text-slate-400">
                    {Math.round(lane.share * 100)}%
                  </p>
                </div>
              ))
            ) : (
              <p className="text-sm text-slate-400">Queue up to populate lane trends.</p>
            )}
          </CardContent>
        </Card>
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-5">
        ...
        <Card className="border-white/10 bg-slate-950/80 lg:col-span-2">
          <CardHeader>
            <CardTitle>Trusted Duo Queue</CardTitle>
            <CardDescription>Based on teammates from your last 20 matches</CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col gap-3">
            {duoPartners.length ? (
              duoPartners.map((duo) => (
                ...
                >
                  <div>
                    <p className="text-sm font-semibold text-slate-100">{duo.name}</p>
                    <p className="text-xs text-slate-400">{duo.games} games together</p>
                  </div>
              ...
              <p className="text-sm text-slate-400">Queue with a consistent teammate to unlock insights.</p>
            )}
          </CardContent>
        </Card>
      </section>
      ...
      <section className="grid gap-6">
        <Card className="border-white/10 bg-slate-950/80">
          <CardHeader>
            ...
            <CardDescription>Most played champions across your last 20 games</CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col gap-3">
            {recentChampionPool.length ? (
              recentChampionPool.map((champion) => (
                <div
                  ...
                  className="flex items-center gap-3 rounded-2xl border border-white/10 bg-slate-900/70 px-3 py-2"
                >
                  <div className="relative h-10 w-10 overflow-hidden rounded-2xl border border-violet-400/30">
                    {champion.icon ? (
                      <Image
                        src={champion.icon}
                      ...
                      <div className="h-full w-full bg-violet-500/20" />
                    )}
                  </div>
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-slate-100">{champion.championName}</p>
                    <p className="text-xs text-slate-400">{champion.games} games</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs uppercase tracking-[0.3em] text-slate-500">Pick Share</p>
                    <p className="text-sm font-semibold text-slate-100">
                      {analysis.sample ? Math.round((champion.games / analysis.sample) * 100) : 0}%
                    </p>
                    <p className="text-xs text-emerald-300">{Math.round(champion.winRate * 100)}% WR</p>
                  </div>
                </div>
              ))
            ) : (
              <p className="text-sm text-slate-400">Finish a few more matches to surface your pool trends.</p>
            )}
          </CardContent>
        </Card>
      </section>
